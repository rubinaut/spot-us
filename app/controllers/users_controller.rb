class UsersController < ApplicationController
  ...
  def list
    ...
    if @filter=='donated' || @filter.blank?  
      user_ids_all = Donation.paid.by_network(current_network).find(:all, :group=>"donations.user_id").map(&:user_id)
      ...
      user_ids_all = Donation.paid.by_network(current_network).find(:all, :conditions=>"users.type='Organization'", :group=>"donations.user_id", :include=>:user).map(&:user_id)
      @items = Donation.by_network(current_network).paginate(:page => params[:page], :select=>"donations.*, max(donations.id) as max_id", 
        :conditions=>["donations.user_id in (?)", user_ids_all], :group=>"donations.user_id", :order=>'max_id desc')
    elsif @filter=='reporters'
      user_ids_all = Pitch.by_network(current_network).browsable.find(:all, :group=>"news_items.user_id").map(&:user_id)
  ...
  def create
    ...
    if @user.save
      unless @user.organization?
        ...
        flash_and_redirect(:success, "Welcome to #{SITE_NAME}!", return_path)
      else
  ...
  def activate
    ...
    if current_user
      ...
      flash[:success] = "You have successfully activated your account - welcome to #{SITE_NAME}!"
